...
__device__ inline float devIoU(float const * const a, float const * const b) {
  ...
  float left_o = max(a[4], b[4]), right_o = min(a[6], b[6]);
  float top_o = max(a[5], b[5]), bottom_o = min(a[7], b[7]);
  float width_o = max(right_o - left_o + 1, 0.f), height_o = max(bottom_o - top_o + 1, 0.f);
  float interS_o = width_o * height_o;
  float Sa_o = (a[6] - a[4] + 1) * (a[7] - a[5] + 1);
  float Sb_o = (b[6] - b[4] + 1) * (b[7] - b[5] + 1);

  return (interS / (Sa + Sb - interS)) * (interS_o / (Sa_o + Sb_o - interS_o));
  //return (interS + interS_o) / ((Sa + Sb - interS) + (Sa_o + Sb_o - interS_o));
}

__global__ void co_nms_kernel(const int n_boxes, const float nms_overlap_thresh,
...
extern "C" int ApplyCONMSGPU(int* keep_out, const float* boxes_dev, const int boxes_num,
  ...
  co_nms_kernel<<<blocks, threads>>>(boxes_num,
                                  nms_overlap_thresh,
                                  boxes_dev,
                                  mask_dev);
