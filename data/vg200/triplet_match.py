...
from data.vg200.utils.tools import vecDist, db2vg, cleaneddb2vg
from data.vg200.utils.word_embeddings import load_word_vectors, obj_edge_vectors
import os
...
import sys
import string
from data.vg200.cleanse_raw_vg import make_alias_dict
...
def triplet_match(wv_dir=GLOVE_DIR, wv_type='glove.6B', wv_dim=300, include_rel=True):
    ...
    dbidx2vgidx, vgidx2dbidx = cleaneddb2vg() #db2vg()
    ...
    with open(osp.join(DATA_DIR, 'cleanse_triplet_match.json'), 'w') as f:
        json.dump(sgs, f)
