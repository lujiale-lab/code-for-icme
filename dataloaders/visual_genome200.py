...
from config import VG_IMAGES, IM_DATA_FN, VG200_SGG_FN, VG200_SGG_DICT_FN, BOX_SCALE, IM_SCALE, SAL_SCALE, PROPOSAL_FN, SALIENCY_FN, DEPTH_FN
...
def vg_collate(data, num_gpus=3, is_train=False, mode='det'):
    ...
    blob = Blob(mode=mode, is_train=is_train, num_gpus=num_gpus,
                batch_size_per_gpu=len(data) // num_gpus, sal=True)
    for d in data:
        blob.append(d)
    blob.reduce()
    return blob
...
class VGDataLoader(torch.utils.data.DataLoader):
    ...
    @classmethod
    def splits(cls, train_data, val_data, batch_size=3, num_workers=8, num_gpus=3, mode='det',
               **kwargs):
        assert mode in ('det', 'rel')
        train_load = cls(
            dataset=train_data,
