...
from config import VG_IMAGES, IM_DATA_FN, VG200_SGG_FN, VG200_SGG_DICT_FN, BOX_SCALE, IM_SCALE, SAL_SCALE, PROPOSAL_FN, \
    SALIENCY_FN, DEPTH_FN, CAPTIONS_INFO, CAPTIONS_FN
...
class VG200_Keyrel_captions(Dataset):
    def __init__(self, mode, roidb_file=VG200_SGG_FN, dict_file=VG200_SGG_DICT_FN,
                 image_file=IM_DATA_FN, saliency_file=SALIENCY_FN, depth_file=DEPTH_FN, filter_empty_rels=True, num_im=-1, num_val_im=5000,
                 filter_duplicate_rels=True, filter_non_overlap=True, captions_info_file=CAPTIONS_INFO, captions_file=CAPTIONS_FN,
                 seq_per_img=5, use_proposals=False):
        """
        ...
        self.ind_to_classes, self.ind_to_predicates, self.subset_dbidxes = load_info(dict_file)
        self.filenames, self.coco_ids = load_image_filenames(image_file)
        self.filenames = [self.filenames[i] for i in self.subset_dbidxes]  # re arange the filename
        self.filenames = [self.filenames[i] for i in np.where(self.split_mask)[0]]
        self.coco_ids = [self.coco_ids[i] for i in self.subset_dbidxes]    # type: int
        self.coco_ids = [self.coco_ids[i] for i in np.where(self.split_mask)[0]]
    ...
    def __getitem__(self, index):
        ...
        # load salinecy map
        saliency_unpadded = self.sal_h5[self.subset_dbidxes[self.image_index[index]]]  # 512, 512
        saliency_unpadded = Image.fromarray(saliency_unpadded)

        # load depth map
        depth_unpadded = self.depth_h5[self.subset_dbidxes[self.image_index[index]]]  # 512, 512
        depth_unpadded = Image.fromarray(depth_unpadded)
        ...
        if flipped:
            ...
            saliency_unpadded = saliency_unpadded.transpose(Image.FLIP_LEFT_RIGHT)
            depth_unpadded = depth_unpadded.transpose(Image.FLIP_LEFT_RIGHT)
        ...
        key_rel_idxes = self.key_rel_idxes[index].copy()
        if self.filter_duplicate_rels:
            ...
            for k, v in all_rel_sets.items():
                vtmp = np.array(v)
                if len(np.where(vtmp[:, 1] == 1)[0]) == 0: # no key
                    gt_rels.append((k[0], k[1], np.random.choice(vtmp[:, 0])))
                else:
                    for ix in np.where(vtmp[:, 1] == 1)[0]:
                        if (k[0], k[1], vtmp[ix, 0]) not in gt_rels:
                            new_key_rel_idxes.append(len(gt_rels))
                            gt_rels.append((k[0], k[1], vtmp[ix, 0]))
            #gt_rels = [(k[0], k[1], np.random.choice(v)) for k, v in all_rel_sets.items()]
            gt_rels = np.array(gt_rels)
            key_rel_idxes = np.array(new_key_rel_idxes)

        # get captions labels
        seq_labels = np.zeros([self.seq_per_img, self.seq_length+2], dtype=np.int)
        mask_labels = np.zeros([self.seq_per_img, self.seq_length+2], dtype=np.float32)
        start_idx = self.label_start_ix[index] - 1 # label_start_ix starts from 1
        end_idx = self.label_end_ix[index] - 1
        ncap = end_idx - start_idx + 1
        assert ncap > 0, 'an image does not have any label.'
        if ncap < self.seq_per_img:
            seq = np.zeros([self.seq_per_img, self.seq_length], dtype=np.int)
            for q in range(self.seq_per_img):
                ix = random.randint(start_idx, end_idx)
                seq[q, :] = self.h5_label_file['labels'][ix, :self.seq_length]
        else:
            ix = random.randint(start_idx, end_idx - self.seq_per_img + 1)
            seq = self.h5_label_file['labels'][ix: ix+self.seq_per_img, :self.seq_length]
        seq_labels[:, 1:self.seq_length+1] = seq
        # generate mask
        nonzeros = np.array(list(map(lambda x: (x!=0).sum()+2, seq_labels)))
        for ix, row in enumerate(mask_labels):
            row[:nonzeros[ix]] = 1  # from <start> to <end>

        entry = {
            'img': self.transform_pipeline(image_unpadded),
            'sal_map': self.sal_transform_pipeline(saliency_unpadded),
            'depth_map': self.depth_transform_pipeline(depth_unpadded),
            'img_size': im_size,
            'gt_boxes': gt_boxes,
...
def load_image_filenames(image_file, image_dir=VG_IMAGES):
    ...
    for i, img in enumerate(im_data):
        ...
        filename = os.path.join(image_dir, sub_dir[0], basename)
        if os.path.exists(filename):
            fns.append(filename)
        else:
            filename = os.path.join(image_dir, sub_dir[1], basename)
            assert os.path.exists(filename)
            fns.append(filename)
        coco_ids.append(img['coco_id'])
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    return split_mask, boxes, gt_classes, relationships, key_relationships
...
def load_info(info_file):
    ...
    subset_dbidxes = info['subset_dbidx']

    return ind_to_classes, ind_to_predicates, subset_dbidxes


def vg_collate(data, num_gpus=3, is_train=False, mode='det'):
    assert mode in ('det', 'rel')
    blob = Blob(mode=mode, is_train=is_train, num_gpus=num_gpus,
                batch_size_per_gpu=len(data) // num_gpus, keyrel=True, sal=True, need_caption=True)
    for d in data:
        blob.append(d)
    blob.reduce()
    return blob
