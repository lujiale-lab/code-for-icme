...
class VG200_Keyrel(Dataset):
    def __init__(self, mode, roidb_file=VG200_SGG_FN, dict_file=VG200_SGG_DICT_FN,
        ...
        self.coco_ids = [self.coco_ids[i] for i in self.subset_dbidxes]
...
def vg_collate(data, num_gpus=3, is_train=False, mode='det'):
    ...
    blob = Blob(mode=mode, is_train=is_train, num_gpus=num_gpus,
                batch_size_per_gpu=len(data) // num_gpus, keyrel=True, sal=True)
    for d in data:
        blob.append(d)
    blob.reduce()
    return blob
